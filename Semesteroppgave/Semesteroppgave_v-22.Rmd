...
```
**Comment**  
We see from Table 4 that the highest sales revenue comes from the price group `r amount_shares_pricegroup_mnd [1,1]`. The revenues were $ `r (sales_most_pricegroup_mnd [1,3])/1000`. The profit from sales in this price group was `r (profit_most_pricegroup_mnd [1,5])/1000`. All numbers are in thousands of $.  
...
**Comment**  
From table 5 we see that the item that sells the most is `r bestselling_pgr_5[1,1]` It was sold `r bestselling_pgr_5[1,3]` units of it in this month.
...
We see from Table 6 that the highest sales revenue comes from the item `r amount_shares_item_mnd [1,1]`. The Sales revenue were $ `r (Shares_most_item_mnd [1,4])/1000`. The profit from sales of this item  was `r (profit_share_most_item_mnd  [1,6])/1000`. All numbers are in thousands of $.  
<br/>
...
**Comment**  
We see from Figure 3 that the largest Sales revenue is achieved by `r sales_most_item_mnd [1,1]`. The sales revenue from this item amounts to $`r (sales_most_item_mnd[1,4])/1000`. Numbers are in thousands of $.     
<br/>

#### **Making a plot of Sales of mostselling items as shares of total Sales:**

```{r plot figure 3a mostselling items shares}
...
**Comment**   
We see from Figure 3a that the largest share of total Sales is achieved by `r Shares_most_item_mnd [1,1]`. This item has `r Shares_most_item_mnd [1,5]` percent of total Sales.  
<br/>

  
#### **Making a plot of profit on mostselling items:**
...
**Comment**  
We see from Figure 4 that the largest profit is achieved from sales in the price group `r profit_most_item_mnd[1,1]`. This item has `r profit_most_item_mnd[1,7]` percent of total Profit.  
<br/>
...
Store_mnd <- All_df_rev %>% 
  ...
  group_by(Store_Num,Store_Name, Store_Location, Store_Drive_Through, Store_Competition_Fastfood,
           County_Total_Crime_Rate, County_Unemployment_Rate, `County_Non-Hispanic_White_pct`,                                          County_Hispanic_Native_American_pct, County_Hispanic_White_pct, `County_Non-Hispanic_Native_American_pct`,                   All_Other_Groups_pct) %>%
  
  summarise(Store_Name_Sold = sum(Sold), Store_Name_Sale = sum(Sales)/1000, 
            Store_Name_Profit = sum(Profit)/1000) %>% 
  # making shares NB får beskjed om at Store_mnd ikke fins. Har tatt ut Store_mnd så alle andelkolonner viser 100
  mutate(store_share_sold_mnd = round(100 *(Store_Name_Sold/sum(Store_Name_Sold)),1),
         store_share_sales_mnd = round(100 *(Store_Name_Sale/sum(Store_Name_Sale)),1),
         store_share_profit_mnd = round(100 *(Store_Name_Profit/sum(Store_Name_Profit)),1)) %>% 
  ...
  rename(other_demo_groups_pct = All_Other_Groups_pct,
         ...
         sold_share = store_share_sold_mnd,
         sales = Store_Name_Sale,
         sales_share = store_share_sales_mnd,
         profit = Store_Name_Profit,
         profit_share = store_share_profit_mnd) %>% 
  ungroup()
Store_mnd
```
```{r}
...
**Comment**  
We see from Table 7 that the store with the largest sales was `r Store_most_sales[1,2]`. The amount was `r Store_most_sales[1,5]` and was `r Store_most_sales[1,6]` percent of the total sales for all the stores.  
