...
```
Tenkte å lage det slik at man kunne velge butikk/butikknr og velge uke(r) for å få de tallene man ønsker
...
Større avstand - avsnitt    
    
#### **Sorting the goods after price groups:**
Due to the large breadth of the product range, it has been chosen to take the product prices as a starting point for the analysis and group them.  
...
**Comment**  
We see from Figure 2 that the largest profit is achieved from sales in the price group `r profit_most [1,1]`. The total profit here is $ `r profit_most [1,4]`. The first price group is negative. The reason for this is that it also contains refunds etc. who will
...
Table 2 shows the 10 items that sell the most. Sales for these 10 are total $ `r sum(Goods_most$Sales)` and account for `r round(100 *(sum(Goods_most$Sales)/sum(sales_14_20$Sales)),1)` percent of total sales.

It appears from Table 2 that most of `r Goods_most [1,2]` is sold. `r Goods_most[1,4]` units were sold, and that account for $`r Goods_most[1,8]` in sale. The sale of `r Goods_most[1,2]` accounts for `r round(100 *(Goods_most[1,8]/sum(sales_14_20$Sales)),1)` percent of total sale  
...
sales_mnd <-All_df_rev %>% 
  ...
  group_by(Description, Price, Sold, Cost, Sales,Profit, Margin) %>% 
  summarise(Sold, Sales) %>% 
  ungroup()
sales_mnd
```

de totale salgsinntektene var $ ......
#FUNKER IKKE?
# sum of variables:
sum(sales_mnd$Sales)
...
De totale salgsinntektene for alle utsalgene i denne måneden var $ `r sum(sales_price_gr_mnd$Tot_sales)`.
Samlet fortjeneste i perioden var $ `r sum(sales_price_gr_mnd$Tot_profit)`.
...
We see from Table 3 that the highest sales revenue comes from the price group `r sales_most_mnd [1,1]`. In this price group, goods were sold for a total of $ `r sales_most_mnd [1,3]`. The profit from sales in this price group was `r sales_most_mnd [1,5]`.
...
The total profit here is $ `r profit_most [1,4]`. The first price group is negative. The reason for this is that it also contains refunds etc. who will negatively affect profits.



Resten er kladd som ligger på skriftet i R
